...
// Uses only baseline schema fields so it remains compatible during staged migrations.
...
$chatStmt = $pdo->prepare(
    'SELECT sn.id AS node_id, sn.message_text, sn.image_url, sn.response_type,
            sn.station_id, sn.puzzle_id, sn.points, sn.type,
            tsl.delivered_at, tsl.responded_at, tsl.team_response,
            tsl.is_completed, tsl.attempts
     FROM story_nodes sn
     INNER JOIN team_story_log tsl ON tsl.node_id = sn.id
     WHERE tsl.team_id = ?
