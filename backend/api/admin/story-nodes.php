...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt->execute([
        ...
        $body['puzzle_id'] ?? null,
        $body['reveals_suspect_id'] ?? null,
        (int)($body['points'] ?? 0),
        !empty($body['is_root']) ? 1 : 0,
        $body['related_node_id'] ?? null,
        $body['proactive_trigger'] ?? 'none',
        $body['proactive_after_minutes'] ?? null,
        $body['proactive_after_attempts'] ?? null,
    ]);
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    ...
    $stmt->execute($params);
    if ($stmt->rowCount() === 0) {
        jsonError(404, 'Knoten nicht gefunden');
    }
