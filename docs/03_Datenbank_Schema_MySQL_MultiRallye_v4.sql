-- Viking-Schatz Rallye: Multi-Rallye-Datenbankschema (Version 4.0 + kumulative Migrationen)
-- Ersetzt: 03_Datenbank_Schema_MySQL_MultiRallye_v3.sql (bitte archivieren)
-- STAND 10.09.2026, 01:40 Uhr: Dieses Dokument enthaelt die Basis v4.0 (Phase A) PLUS
-- die kumulativen Aenderungen aus den Migrationen 002-004 (Phase E, F, Lead-only-Unlock).
-- Die tatsaechliche Ausfuehrungsreihenfolge auf der produktiven DB ist:
--   1. Diese Basis-Datei (v4.0, Phase A)
--   2. backend/migrations/002_ermittler_chat_phase_e.sql
--   3. backend/migrations/003_ermittler_chat_phase_f.sql
--   4. backend/migrations/004_lead_only_station_unlock.sql
-- Dieses Dokument dient nur als konsolidierte Referenz -- die einzelnen
-- Migrationsdateien in backend/migrations/ bleiben die Ausfuehrungs-Quelle.
...
-- teams.avatar_url: urspruenglich VARCHAR(64) als Icon-Schluessel gedacht (Phase-A-Spezifikation),
-- in Phase F tatsaechlich als VARCHAR(255) Datei-Pfad implementiert (echter Bild-Upload statt
-- Icon-Auswahl). Nachfolgende Deklaration spiegelt den TATSAECHLICH implementierten Stand wider.
CREATE TABLE teams (
  id INT AUTO_INCREMENT PRIMARY KEY, rallye_id INT NOT NULL,
  start_code VARCHAR(12) UNIQUE NOT NULL, name VARCHAR(255) NULL,
  avatar_url VARCHAR(255) NULL COMMENT 'Datei-Pfad zum hochgeladenen Team-Avatar (Phase F)',
...
-- WICHTIG: discovery_mode nutzt DURCHGAENGIG Unterstriche ('lead_only', nicht 'leadonly').
-- Siehe 00_Project_Brief_Entscheidungslog_v3.md fuer einen bekannten Frontend-Bug, bei dem
-- diese Konvention nicht eingehalten wurde.
...
-- unlock_source um 'chat' erweitert (Migration 004, Lead-only-Station-Unlock via Chat).
CREATE TABLE station_unlocks (
  id INT AUTO_INCREMENT PRIMARY KEY, team_id INT NOT NULL, station_id INT NOT NULL,
  unlock_source ENUM('qr','gps','manual','chat') NOT NULL,
...
-- attempt_number = 0 ist NICHT reserviert (fruehere Idee eines separaten Hint-Buchungs-Datensatzes
-- wurde verworfen -- der Punktabzug fuer Hinweise fliesst stattdessen direkt in points_earned der
-- regulaeren Loesungs-Zeile ein, siehe puzzles/submit.php).
...
-- response_type um 'photo_ref' (Phase E), media_type/media_url ergaenzt (Phase F).
CREATE TABLE story_nodes (
  id INT AUTO_INCREMENT PRIMARY KEY, rallye_id INT NOT NULL,
  type ENUM('info','answer','twist','accusation') NOT NULL DEFAULT 'info',
  message_text TEXT NOT NULL, image_url VARCHAR(255) NULL,
  media_type ENUM('none','audio_ref','video_ref') NOT NULL DEFAULT 'none',
...
-- unlocks_station_id ergaenzt (Migration 004, Lead-only-Station-Unlock via Chat).
CREATE TABLE story_node_options (
  ...
  INDEX idx_options_node (node_id), INDEX idx_options_unlocks_station (unlocks_station_id)
) ENGINE=InnoDB;

-- team_response speichert seit dem Bugfix vom 09.09.2026 abends bei response_type='buttons'
-- das Options-LABEL (nicht die numerische Options-ID) -- lesbare Anzeige unter "Deine Antwort".
...
-- Team-/Admin-Endpunkte selektieren seit dem Bugfix vom 09.09.2026 abends explizit
-- team_id, team_name, stations_completed, total_points, total_hints_used MIT eigener
-- ORDER BY (siehe 02_Technische_Spezifikation_PHP_v3.md).
